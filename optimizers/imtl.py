...
class IMTL(MTLOptimizer):
    ...
    def custom_backwards(self, objectives):
        ...
        if self.learn_scaling:
            ...
            if self.st == None:
                self.st = torch.zeros(
                    (len(objectives),) + tuple(objectives[0].shape), requires_grad=True, device=objectives[0].device
                )
        ...
        if self.learn_scaling:
            imtl_l_weights = torch.exp(self.st).detach().cpu().numpy()
            self.st_optimizer.step()  # the grad is accumulated above
            self.st_optimizer.zero_grad()
        else:
            imtl_l_weights = np.ones(len(objectives))

        weight_data = {}
        weight_data.update(self.weight_to_dict(alpha.cpu().numpy(), "IMTL-G_weights"))
        weight_data.update(self.weight_to_dict(imtl_l_weights, "IMTL-L_weights"))
