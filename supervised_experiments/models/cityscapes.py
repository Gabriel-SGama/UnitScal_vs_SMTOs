...
class DeepLabHead(nn.Sequential):
    def __init__(self, in_channels, num_classes, log_softmax_out=True, norm_out=False, img_size=[128, 256], p=0.0):
        super(DeepLabHead, self).__init__(
            ASPP(in_channels, [12, 24, 36], p=p),
    ...
    def forward(self, x, mask=None):
        ...
        if self.log_softmax_out:
            ...
            return rep / torch.norm(rep, p=2, dim=1, keepdim=True), None
        else:
            return rep, None
...
class ASPP(nn.Module):
    def __init__(self, in_channels, atrous_rates, p=0.0):
        ...
        layers = [nn.Conv2d(5 * out_channels, out_channels, 1, bias=False), nn.BatchNorm2d(out_channels), nn.ReLU()]
